...
public class Enemy extends Collisonable {
    ...
    private final BufferedImage bulletImg = Helper.loadImg("resources/Bullet.gif");
    ...
    public Enemy(int type, int rank, BufferedImage img){
        ...
        x = Math.random()* GameConstants.GAME_SCREEN_WIDTH /2 + GameConstants.GAME_SCREEN_WIDTH/4;
        randNum =(int) (Math.random()*10);
        y = Helper.isOdd(randNum)? GameConstants.GAME_SCREEN_HEIGHT+r:-r;
...
//    }
    ...
    public void update(){
        ...
        if(!ready){
            if(x>r && x<GameConstants.GAME_SCREEN_WIDTH-r && y>r && y<GameConstants.GAME_SCREEN_HEIGHT -r){
                ready = true;
            }
        }
        ...
        if(x>GameConstants.GAME_SCREEN_WIDTH- r && dx>0) {dx=-dx; angle= (180-angle); }
        if(y>GameConstants.GAME_SCREEN_HEIGHT - r && dy>0) {dy = -dy; angle = -angle;}
